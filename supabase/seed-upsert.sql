-- =============================================================================
-- Netflix Portfolio v2 — Upsert Seed (idempotent)
-- Run this AFTER schema.sql in the Supabase SQL editor.
-- Safe to re-run: uses ON CONFLICT DO UPDATE so it always reflects latest data.
-- Source of truth — supersedes seed.sql and seed-fresh.sql.
-- =============================================================================

-- ─── MISSING TABLES (added after initial schema deploy — safe to re-run) ────
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'images' AND policyname = 'public_read') THEN
    CREATE POLICY "public_read" ON images FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'images' AND policyname = 'admin_all') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'contact_info' AND policyname = 'public_read') THEN
    CREATE POLICY "public_read" ON contact_info FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'contact_info' AND policyname = 'admin_all') THEN
    CREATE POLICY "admin_all"  ON contact_info FOR ALL USING (auth.role() = 'authenticated');
  END IF;
END $$;

-- ─── ENSURE UNIQUE INDEXES EXIST (idempotent — safe to re-run) ──────────────
CREATE UNIQUE INDEX IF NOT EXISTS skills_name_key
  ON skills (name);
CREATE UNIQUE INDEX IF NOT EXISTS certifications_title_provider_key
  ON certifications (title, provider);
CREATE UNIQUE INDEX IF NOT EXISTS experience_company_role_key
  ON experience (company, role);
CREATE UNIQUE INDEX IF NOT EXISTS about_sections_section_key_key
  ON about_sections (section_key);
CREATE UNIQUE INDEX IF NOT EXISTS site_settings_key_key
  ON site_settings (key);
CREATE UNIQUE INDEX IF NOT EXISTS contact_info_key_key
  ON contact_info (key);
CREATE UNIQUE INDEX IF NOT EXISTS content_variants_entity_profile_field_key
  ON content_variants (entity_type, entity_id, profile_type, field_name);
...
ON CONFLICT (slug) DO UPDATE SET
  ...
  screenshot_url = COALESCE(NULLIF(projects.screenshot_url, ''), EXCLUDED.screenshot_url),
...
-- AI / ML
('LangChain',     'AI / ML',  'Agents, chains, RAG pipelines',                     1, true),
('Gemini AI',     'AI / ML',  'Gemini API, embeddings, multimodal',               2, true),
('ChromaDB',      'AI / ML',  'Vector store, embeddings, similarity search',       3, true),
('OpenCV',        'AI / ML',  'Computer vision, image processing',                 4, true),
-- Tools
...
(
  ...
  'LinkedIn Learning', '2023-09-01',
  null, 6, true
),
(
  'Programming with JavaScript',
  'Coursera (Meta)', '2023-03-01',
  null, 7, true
),
(
  'Introduction to Structured Query Language (SQL)',
  'University of Michigan', '2022-10-01',
  null, 8, true
),
(
...
(
  ...
  'Codio (Coursera)', '2022-08-01',
  null, 10, true
),
(
  'Problem Solving (Intermediate)',
  'HackerRank', '2023-01-01',
  null, 11, true
),
(
  'JavaScript (Intermediate)',
  'HackerRank', '2022-11-01',
  null, 12, true
)
ON CONFLICT (title, provider) DO UPDATE SET
  ...
  verification_url   = COALESCE(EXCLUDED.verification_url, certifications.verification_url),
  logo_url           = COALESCE(NULLIF(certifications.logo_url, ''), EXCLUDED.logo_url),
  display_order      = EXCLUDED.display_order,
  visible            = EXCLUDED.visible,
  updated_at         = now();
...
BEGIN
  INSERT INTO content_variants (entity_type, entity_id, profile_type, field_name, content) VALUES
  (
  ...
  ON CONFLICT (entity_type, entity_id, profile_type, field_name) DO UPDATE SET
    content    = EXCLUDED.content,
    updated_at = now();
END $$;
